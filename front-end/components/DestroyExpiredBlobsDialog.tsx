...
import { useBlobDestroy } from "@/hooks/useBlobDestroy";
...
interface DestroyExpiredBlobsDialogProps {
  isOpen: boolean;
  onClose: () => void;
  selectedItems: WalrusBlob[];
  onSuccess?: () => void;
}

export function DestroyExpiredBlobsDialog({
  isOpen,
  onClose,
  selectedItems,
  onSuccess,
}: DestroyExpiredBlobsDialogProps) {
  const [confirmed, setConfirmed] = useState(false);
  const { destroyBlobs, isDestroying } = useBlobDestroy();
  ...
  // Convert items to blob data format
  const blobData = useMemo(() => {
    return selectedItems.map((item) => ({
      objectId: item.objectId,
      blobId: item.blobId,
      storageSize: item.storage.storageSize,
      startEpoch: item.storage.startEpoch,
      endEpoch: item.storage.endEpoch,
    }));
  }, [selectedItems]);
  ...
  const handleDestroy = async () => {
    ...
    try {
      ...
      toast({
        title: "Success!",
        description: `Successfully destroyed ${selectedItems.length} expired blob${selectedItems.length !== 1 ? 's' : ''}`,
      });

      // Call onSuccess callback to refresh the blob list
      onSuccess?.();

      // Close the dialog
      handleClose();
    } catch (error) {
      const errorMessage = error instanceof Error ? error.message : 'Failed to destroy blobs';
      toast({
        title: "Error",
      ...
      <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto border-2 border-red-500 shadow-[4px_4px_0px_0px_rgba(239,68,68,1)]">
        <DialogHeader>
          <DialogTitle className="text-2xl font-black text-red-600">
            Destroy Expired Blobs
          </DialogTitle>
          <DialogDescription>
            You have selected {selectedItems.length} expired blob
        ...
        <div className="space-y-6 py-4">
          ...
          <div className="backdrop-blur-md bg-red-500/10 border-2 border-red-500 rounded-xl p-4">
            ...
            <p className="text-red-600 text-sm mt-2">
              Destroying blobs will permanently delete them and their storage from the blockchain.
            </p>
          </div>
          ...
          <div className="flex items-start space-x-3 backdrop-blur-md bg-red-500/5 border-2 border-red-500 rounded-xl p-4">
            ...
            <div className="flex flex-col gap-1">
              ...
              <p className="text-xs text-gray-600">
                These blobs will be permanently deleted and cannot be recovered
              </p>
            </div>
          </div>
        </div>
